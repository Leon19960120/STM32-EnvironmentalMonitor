...
//----------------------------------------------------加密-------------------------------------------------------------------------
	...
	hmac_sha1((unsigned char *)access_key_base64, olen,
				(unsigned char *)string_for_signature, strlen(string_for_signature),
				(unsigned char *)hmac_sha1_buf);
				
	//UsartPrintf(USART_DEBUG, "hmac_sha1 returned\r\n");
...
//		Delay_ms(500);
//	
//	OneNET_Authorization("2018-10-31", PROID, 1956499200, key, NULL,
...
unsigned char OneNet_FillBuf(char *buf)
{
	...
	memset(text, 0, sizeof(text));
	sprintf(text, "\"humidity\":{\"value\":%d},", humidity);
	strcat(buf, text);
...
//	// ====================== 新增：串口打印完整JSON包 ======================
//    UsartPrintf(USART_DEBUG, "=====================================\r\n");
//    UsartPrintf(USART_DEBUG, "OneNet 上报JSON包:\r\n");
//    UsartPrintf(USART_DEBUG, "长度: %d 字节\r\n", strlen(buf));
//    UsartPrintf(USART_DEBUG, "内容: %s\r\n", buf);
//    UsartPrintf(USART_DEBUG, "=====================================\r\n");
...
//	函数名称：	OneNet_SendData
//
//	函数功能：	上传数据到新版 OneNET 平台（物模型标准 Publish）
//==========================================================
void OneNet_SendData(void)
{
    // 1. 维持原样：发送 PING 心跳包报平安
    MQTT_PACKET_STRUCTURE pingPkt = {NULL, 0, 0, 0};
    if (MQTT_PacketPing(&pingPkt) == 0) {
        ESP8266_SendData(pingPkt._data, pingPkt._len);
        MQTT_DeleteBuffer(&pingPkt);
    }
    ...
    // 填充物模型 JSON 字符串 (包含 id, version, params)
    OneNet_FillBuf(buf); 
    
    // ?? 关键点：组合新版 OneNET 专属的物模型数据上报 Topic
    snprintf(topic, sizeof(topic), "$sys/%s/%s/thing/property/post", PROID, DEVICE_NAME);
    
    // 串口打印一下，方便你在电脑上肉眼观察发了什么
    //UsartPrintf(USART_DEBUG, "Publish Topic: %s\r\n", topic);
    //UsartPrintf(USART_DEBUG, "Publish Payload: %s\r\n", buf);
    
    // ?? 核心：调用标准的 MQTT 发布函数，清清爽爽地发出去
    OneNET_Publish(topic, buf);
}
//void OneNet_SendData(void)
//{
//    char buf[256];
//    char topic[128];
//    OneNet_FillBuf(buf);   // 你的函数已生成 {"id":"123","version":"1.0","params":{"Temp":{"value":28},"humidity":{"value":49}}}
//    snprintf(topic, sizeof(topic), "$sys/%s/%s/thing/property/post", PROID, DEVICE_NAME);
//    OneNET_Publish(topic, buf);
//}
