...
#define CMD_TOPIC_PREFIX		"$creq"
	...
	{
		//UsartPrintf(USART_DEBUG, "MQTT_NewBuffer: size=%lu, _data=%p, _size=%lu\n", size, mqttPacket->_data, mqttPacket->_size);
		uint32 i = 0;
...
//	char topic_buf[48];
//	
//	snprintf(topic_buf, sizeof(topic_buf), "$sys/%s/%s/thing/property/post", pro_id, dev_name);//$sys/Q0l3rvndi9/Test/thing/property/post
//	
...
uint1 MQTT_PacketSaveData(const int8 *pro_id, const char *dev_name,
                            int16 send_len, int8 *send_buf, MQTT_PACKET_STRUCTURE *mqttPacket)
{
    char topic_buf[64];

    // 1. 正确的物模型上报主题（up不是post）
    snprintf(topic_buf, sizeof(topic_buf), "$sys/%s/%s/thing/property/post", pro_id, dev_name);

    // 2. 构造MQTT头部和主题（QoS必须是0）
    if(MQTT_PacketPublish(MQTT_PUBLISH_ID, topic_buf, NULL, send_len, MQTT_QOS_LEVEL0, 0, 0, mqttPacket) == 0)
    {
        // ====================== 最关键的一行！！！ ======================
        // 把你的JSON数据复制到MQTT包的末尾
        memcpy(mqttPacket->_data + mqttPacket->_len, send_buf, send_len);
        mqttPacket->_len += send_len;
